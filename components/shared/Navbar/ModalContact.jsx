...
export const ModalContact = ({ theme, contactModalIsOpen, closeContactModal }) => {
  ...
  const sendEmail = (e) => {
    ...
    emailjs.sendForm(process.env.NEXT_PUBLIC_SERVICE_ID_EMAIL, process.env.NEXT_PUBLIC_TEMPLATE_ID_EMAIL, form.current, process.env.NEXT_PUBLIC_USER_ID_EMAIL)
    .then((result) => {
      if (result.text === 'OK') {
                        ...
                        >
                          <div className="absolute inset-0 z-10">
                            <div className="flex flex-col items-center justify-center w-full h-full space-y-3 font-medium text-white rounded-xl to-green-500 from-green-600 bg-gradient-to-r">
                              ...
                              <p className="text-2xl">{t.form.success_message}</p>
                            </div>
                          </div>
                        </Transition>
                        <form ref={form} onSubmit={sendEmail}>
                          <div className="space-y-3 dark:text-white p-0.5">
                            ...
                            <div className="w-full">
                              {
                                errorReCaptcha ? (
                                  <span className="text-sm text-red-500">{t.form.error_recaptcha}</span>
                                ) : (<></>)
                              }
                              <ReCAPTCHA
                                ref={recaptchaRef}
